...
def trans(s:str,args:argparse.Namespace):
    ...
    if args.schema_name is not None:
        input = attach_schema_name(input,args.schema_name,read_qtrack_table_list())    
    return input
...
def make_was_xml(was_xml_path:str,was_group_query:str,was_program_query:str,engine:sa.engine,logger:logging.Logger,args:argparse.Namespace):
    ...
    try:
        ...
        for idx in range(len(was_rtde_group_df)) :
            ...
            was_rtde_id_df = pd.read_sql_query(was_program_query,con=engine,params=tuple(was_rtde_group_df.values[idx]))
            ...
            with open(was_xml_path+file_name, 'w',encoding='utf-8') as file:
                ...
                for inner_idx in range(len(was_rtde_id_df)) :
                    s = str(DecryptFile.decrypt(was_rtde_id_df['pgm_src'][inner_idx]))
                    s = trans(s,args)
...
def make_group_meta_tag(file,row_df:pd.DataFrame,index:int):
    row_df.loc[index,'pgm_grp_desc'] = make_cdata(str(DecryptFile.decrypt(row_df['pgm_grp_desc'][index])))
    rows = tuple(row_df.values[index])
    file.write("""<rtde_grp """)
...
def make_program_meta_tag(file,row_df:pd.DataFrame,index:int):
        row_df.loc[index,'pgm_desc'] = make_cdata(str(DecryptFile.decrypt(row_df['pgm_desc'][index])))
        rows = tuple(row_df.values[index])
        file.write("""<rtde_pgm """)
...
def make_was_meta_xml(was_xml_path,was_group_meta_query,was_program_meta_query,engine,logger):
    ...
    try:
        ...
        was_rtde_meta_group_df = pd.read_sql_query(was_group_meta_query,con=engine)
        was_rtde_meta_pgm_df = pd.read_sql_query(was_program_meta_query,con=engine)
        ...
        with open(was_xml_path+'rtde_was_meta.xml','w',encoding='utf-8') as file :
            ...
            for idx in range(len(was_rtde_meta_group_df)) :
                file = file + make_group_meta_tag(file,was_rtde_meta_group_df,idx)

            file = functools.reduce(make_group_meta_tag,)

            #프로그램태그 만들기
            for idx in range(len(was_rtde_meta_pgm_df)) :
                file = file + make_program_meta_tag(file,was_rtde_meta_pgm_df,idx)
...
def make_lia_xml(lia_xml_path:str,lia_program_query:str,engine:sa.engine,logger:logging.Logger,args:argparse.Namespace):
    ...
    try: 
        ...
        for idx in range(len(lia_rtde_id_df)) :
            ...
            s = trans(s,args)
            lia_rtde_id_df.loc[idx,'pgm_src'] = make_cdata(s.strip('\n').strip('\r'))
            # XML파일생성 
...
def make_lia_meta_xml(lia_xml_path,lia_group_meta_query,lia_program_meta_query,engine,logger):    
    ...
    try:
        lia_rtde_meta_group_df = pd.read_sql_query(lia_group_meta_query,con=engine)
        lia_rtde_meta_pgm_df = pd.read_sql_query(lia_program_meta_query,con=engine)
        ...
        with open(lia_xml_path+'rtde_lia_meta.xml','w',encoding='utf-8') as file :
            ...
            for idx in range(len(lia_rtde_meta_group_df)) :
                lia_rtde_meta_group_df.loc[idx,'pgm_grp_desc'] = make_cdata(str(DecryptFile.decrypt(lia_rtde_meta_group_df['pgm_grp_desc'][idx])))
            ...
            for idx in range(len(lia_rtde_meta_pgm_df)) :
                lia_rtde_meta_pgm_df.loc[idx,'pgm_desc'] = make_cdata(str(DecryptFile.decrypt(lia_rtde_meta_pgm_df['pgm_desc'][idx])))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--schema_name",'-schema', help="for change schema name",type=str)
    ...
    try :
        ...
        was_rtde_group_sql,was_rtde_id_sql = rtde_program_query(type='was',ais8800_table_name='ais8800',ais8801_table_name='ais8801')
        was_rtde_meta_group_sql = rtde_group_meta_query(type='was',ais8801_table_name='ais8801')
        was_rtde_meta_pgm_sql = rtde_program_meta_query(type='was',ais8800_table_name='ais8800')
        lia_rtde_id_sql = rtde_program_query(type='lia',ais8800_table_name='ais8800',ais8801_table_name='ais8801')
        lia_rtde_meta_group_sql = rtde_group_meta_query(type='lia',ais8801_table_name='ais8801')
        lia_rtde_meta_pgm_sql = rtde_program_meta_query(type='lia',ais8800_table_name='ais8800')
        ...
        if logger.isEnabledFor(level=10) is not True:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=max) as e:
                was_xml_count = e.submit(make_was_xml,was_xml_path,was_rtde_group_sql,was_rtde_id_sql,engine,logger,args)
                e.submit(make_was_meta_xml,was_xml_path,was_rtde_meta_group_sql,was_rtde_meta_pgm_sql,engine,logger)
                lia_xml_count = e.submit(make_lia_xml,lia_xml_path,lia_rtde_id_sql,engine,logger,args)
                e.submit(make_lia_meta_xml,lia_xml_path,lia_rtde_meta_group_sql,lia_rtde_meta_pgm_sql,engine,logger)
            summary(logger,was_xml_count.result(),lia_xml_count.result())

        else:
            was_xml_count = make_was_xml(was_xml_path,was_rtde_group_sql,was_rtde_id_sql,engine,logger,args)
            make_was_meta_xml(was_xml_path,was_rtde_meta_group_sql,was_rtde_meta_pgm_sql,engine,logger)
            lia_xml_count = make_lia_xml(lia_xml_path,lia_rtde_id_sql,engine,logger,args)
            make_lia_meta_xml(lia_xml_path,lia_rtde_meta_group_sql,lia_rtde_meta_pgm_sql,engine,logger)
